...
export default function EditProduct({ params }: EditProductProps) {
  ...
  const handleCharacteristicChange = (
    ...
    const updatedCharacteristics = [
      ...updatedExtraProduct[indexProduct].characteristics,
    ];
  ...
  const AddExtraProductChange = () => {
    ...
    if (extraProduct.length !== 5) {
      ...
      setProduct({
        ...product,
        extraProduct: [...extraProduct, newExtraProduct],
      });
    } else {
  ...
  const DeleteExtraProduct = async (indexProduct: number) => {
    if (indexProduct > 0) {
      const deleteExtraProduct = extraProduct.filter(
        (e: any, i: number) => i !== indexProduct
      );
  ...
  const DeleteСharacteristic = (index: number, indexProduct: number) => {
    const deletechas = extraProduct[indexProduct].characteristics.filter(
      (items: any, i: any) => i !== index
    );
  ...
  useEffect(() => {
    setExtraProduct((prev: any) =>
      oneProduct.extraProduct ? [...oneProduct.extraProduct] : prev
    );
      ...
      <div className="w-full flex flex-col gap-[20px]  rounded-[10px] bg-white px-[20px] pt-[50px] pb-[30px] ">
        ...
        >
          <div className="flex flex-wrap md:flex-nowrap gap-[20px]">
            ...
            >
              <option
                className="px-[20px] rounded-[10px] hover:!text-whiter hover:!bg-black"
                value=""
              >
                Категории
              </option>
              {category.stateCategory
                ? category.stateCategory.map((e: any, i: number) => (
                    <option
                      key={i}
                      className="px-[20px] rounded-[10px] hover:!text-whiter hover:!bg-black"
                      value={e.search_key}
          ...
          {extraProduct.map((productExtra: any, indexProduct: number) => (
            <div
              key={indexProduct}
              style={{ borderColor: productExtra.color }}
              className={`flex flex-col gap-[20px] px-[20px]  border-x-[4px]`}
            >
              <div className="flex flex-wrap md:flex-nowrap gap-[20px]">
                <input
                  ...
                  onChange={(e) =>
                    handleExtraProductFieldChange(indexProduct, e.target)
                  }
                  placeholder="."
                />

                <label
                  htmlFor={`file${indexProduct}`}
                  className="w-full  rounded-[10px] border-grey border-[2px] px-[20px]  py-[15px] placeholder:text-black "
                >
                  <span>
                    выбрано фот {extraProduct[indexProduct].images.length}
                  </span>
                  <input
                    type="file"
                    multiple
                ...
                <div className="w-full relative ">
                  <input
                    ...
                    onChange={(e) =>
                      handleExtraAndNameChange(indexProduct, e.target)
                    }
                  />
              ...
              <div className="flex flex-col gap-[20px] ">
                {productExtra.characteristics.map(
                  (product: any, index: number) => (
                    <div
                      key={index}
                      className="flex flex-wrap md:flex-nowrap gap-[20px] items-center"
                    >
                      <input
                        className="w-full  rounded-[10px] border-grey border-[2px] px-[20px] py-[15px] placeholder:text-black "
                        type="text"
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          DeleteСharacteristic(index, indexProduct)
                        }
